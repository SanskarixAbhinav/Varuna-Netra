...
const Flag = ({ ok, label, reason }) => (
  <div className="flex items-start gap-2 text-xs" data-testid={`playbook-${label.replace(/\s/g, "-").toLowerCase()}`}>
    <span className="mt-0.5 shrink-0 rounded px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider" style={{ color: ok ? "#10B981" : "#FF2A6D", border: "1px solid currentColor" }}>{ok ? "suitable" : "not suitable"}</span>
...
export const Playbook = ({ caseId }) => {
  ...
  return (
    <div className="space-y-4 p-4" data-testid="playbook-panel">
      <div className="rounded border px-3 py-2 text-[11px]" style={{ borderColor: "rgba(255,183,3,0.5)", background: "repeating-linear-gradient(135deg, rgba(255,183,3,0.06) 0 8px, transparent 8px 16px)" }} data-testid="playbook-advisory">
      ...
      <section className="rounded border p-3" style={{ borderColor: "rgba(255,42,109,0.5)" }} data-testid="playbook-tier1">
        <div className="mb-1 flex items-center gap-2"><LifeBuoy size={13} color="#FF2A6D" /><span className="font-display text-sm font-semibold">Tier 1 · {t1.title}</span><span className="ml-auto font-mono text-[10px] uppercase text-rose-300">{t1.priority}</span></div>
        ...
        {pb.tactical_coordinates.length > 0 && (
          <div className="mt-2 grid grid-cols-2 gap-1 font-mono text-[10px]" data-testid="playbook-tactical">
            {pb.tactical_coordinates.map((c) => <div key={c.id} className="flex items-center gap-1.5 rounded bg-slate-900/60 px-2 py-1"><MapPin size={10} color="#FFB703" /><span className="text-amber-300">{c.id}</span> {c.lat.toFixed(4)}, {c.lon.toFixed(4)} <span className="text-slate-500">· {c.role}</span></div>)}
          </div>)}
      </section>
      <section className="rounded border p-3 space-y-1.5" style={{ borderColor: "rgba(157,78,221,0.5)" }} data-testid="playbook-tier2">
        <div className="mb-1 flex items-center gap-2"><FlaskConical size={13} color="#9D4EDD" /><span className="font-display text-sm font-semibold">Tier 2 · {t2.title}</span></div>
      ...
      <section className="rounded border p-3" style={{ borderColor: "rgba(16,185,129,0.5)" }} data-testid="playbook-tier3">
        <div className="mb-1 flex items-center gap-2"><CalendarClock size={13} color="#10B981" /><span className="font-display text-sm font-semibold">Tier 3 · {t3.title}</span></div>
