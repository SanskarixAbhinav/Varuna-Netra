      ...
      <div className="grid grid-cols-3 gap-2">
        {[["confirm", "Confirm", "#10B981", "btn-confirm-analyst-review"], ["reject", "Reject", "#FF2A6D", "btn-reject-analyst-review"], ["needs_more_data", "Needs data", "#FFB703", "btn-needs-data-analyst-review"]].map(([v, l, col, tid]) => (
          <button key={v} data-testid={tid} onClick={() => setDecision(v)} className="rounded border px-2 py-2 font-mono text-[11px] uppercase tracking-wider transition-colors"
            style={{ borderColor: decision === v ? col : "var(--border-highlight)", color: decision === v ? col : "#94A3B8", background: decision === v ? `${col}18` : "transparent" }}>{l}</button>
        ))}
      </div>
      <div>
        <label className="label-mono block mb-1">Vessel {decision === "confirm" && <span style={{ color: "#FF2A6D" }}>*</span>}</label>
        ...
        <div className="flex flex-wrap gap-1.5">
          {Object.entries(reasonCodes || {}).map(([code, desc]) => (
            <button key={code} title={desc} data-testid={`reason-code-${code}`} onClick={() => toggle(code)}
              className={`rounded px-2 py-1 font-mono text-[10px] transition-colors ${codes.includes(code) ? "bg-cyan-400/15 text-cyan-300 border border-cyan-400/50" : "border border-slate-700 text-slate-400 hover:text-slate-100"}`}>{code.split("_")[0]}</button>
          ))}
        </div>
...
const OverrideForm = ({ caseId, candidates, onSaved }) => {
  ...
  return (
    <div className="mt-2 rounded border p-3" style={{ borderColor: "rgba(255,183,3,0.4)", background: "rgba(255,183,3,0.05)" }} data-testid="override-panel">
      ...
      {open && (
        <div className="mt-3 space-y-2 fade-up">
          ...
          <button data-testid="btn-override-submit" disabled={busy || notes.trim().length < 3} onClick={submit} className="rounded px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-slate-950 disabled:opacity-50" style={{ background: "#FFB703" }}>{busy ? "Saving…" : "Apply override"}</button>
        </div>
      )}
    </div>
  );
