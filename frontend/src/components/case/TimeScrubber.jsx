...
export const TimeScrubber = ({ geojson, acquisitionTime, windowAfterHours = 3, cursor, setCursor }) => {
  ...
  return (
    <div className="rounded p-3" style={{ background: "rgba(10,14,23,0.88)", border: "1px solid var(--border-highlight)", backdropFilter: "blur(12px)" }} data-testid="time-scrubber">
      <div className="flex items-center gap-2">
        ...
        <button data-testid="scrubber-reset" onClick={() => { setPlaying(false); setCursor(null); }} title="Show full tracks" className="grid h-7 w-7 place-items-center rounded border text-slate-300 hover:text-white" style={{ borderColor: "var(--border-highlight)" }}><RotateCcw size={12} /></button>
        <div className="relative flex-1">
          ...
          <span className="pointer-events-none absolute -top-1 h-5 w-px" style={{ left: `${acqPct}%`, background: "#FF2A6D" }} title="satellite pass" />
        </div>
      ...
      <div className="mt-1.5 flex items-center justify-between font-mono text-[10px] text-slate-400">
        ...
        <span data-testid="scrubber-relative" style={{ color: rel == null ? "#94A3B8" : rel < 0 ? "#FFB703" : "#FF2A6D" }}>{rel == null ? `${fmtTime(new Date(range.min).toISOString())} → ${fmtTime(new Date(range.max).toISOString())}` : rel < 0 ? `T${rel.toFixed(1)}h before satellite pass` : `T+${rel.toFixed(1)}h after pass`}</span>
      </div>
    </div>
  );
};
