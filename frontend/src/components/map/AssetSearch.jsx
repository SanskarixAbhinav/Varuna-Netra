...
const TYPE_COLOR = { oil_field: "#FFB703", basin: "#F97316", terminal: "#38BDF8", shipping_lane: "#10B981", country: "#94A3B8", eez: "#00F0FF", other: "#CBD5E1" };
...
export const AssetSearch = ({ onSelect, compact }) => {
  ...
  return (
    <div className={`relative ${compact ? "w-72" : "w-96"}`} data-testid="asset-search">
      ...
      {open && hits.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-[1200] mt-1 max-h-80 overflow-y-auto rounded border" style={{ background: "rgba(10,14,23,0.96)", borderColor: "var(--border-highlight)", backdropFilter: "blur(12px)" }} data-testid="asset-search-results">
          {hits.map((h) => (
            <button key={h.id} data-testid={`asset-hit-${h.id}`} onMouseDown={() => pick(h)} className="flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-xs hover:bg-slate-800/70">
              <MapPin size={11} color={TYPE_COLOR[h.type] || "#CBD5E1"} />
              <span className="truncate text-slate-100">{h.name}</span>
              <span className="ml-auto shrink-0 font-mono text-[10px] uppercase tracking-wider" style={{ color: TYPE_COLOR[h.type] || "#CBD5E1" }}>{h.type}</span>
