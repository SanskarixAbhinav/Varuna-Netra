...
export const ProvBadge = ({ p, testId }) => <span data-testid={testId} className="rounded px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider" style={{ color: PROV_COLOR[p] || "#94A3B8", border: `1px solid ${PROV_COLOR[p] || "#94A3B8"}55` }}>{p || "UNVERIFIED"}</span>;
...
export const AoiPanel = ({ onFlyTo, onAoiChange, drawing, setDrawing, drawnGeometry, onDrawnConsumed, compact = false }) => {
  ...
  return (
    <div className="rounded border p-3" style={{ borderColor: "rgba(0,240,255,0.35)", background: "rgba(0,240,255,0.04)" }} data-testid="aoi-panel">
      <div className="mb-2 flex items-center gap-2"><Globe2 size={14} color="#00F0FF" /><h2 className="font-display font-semibold">Investigation AOI · worldwide</h2>
      ...
      {aoi ? (
        <div className="mb-2 rounded border p-2 text-[11px]" style={{ borderColor: "var(--border-default)" }} data-testid="aoi-current">
          <div className="flex items-center gap-2"><Crosshair size={11} color="#00F0FF" /><span className="text-slate-100">{aoi.name}</span><ProvBadge p={aoi.provenance} testId="aoi-provenance" /></div>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5" data-testid="aoi-presets">
        {Object.entries(presets).map(([k, p]) => (
          <button key={k} data-testid={`aoi-preset-${k}`} disabled={busy || !analyst} onClick={() => select({ kind: "preset", preset: k })} className="rounded border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider disabled:opacity-40" style={{ borderColor: k === "sih_mumbai" ? "rgba(255,183,3,0.6)" : "var(--border-highlight)", color: k === "sih_mumbai" ? "#FFB703" : "#CBD5E1" }} title={p.note || p.name}>{k === "sih_mumbai" ? "★ SIH · Mumbai" : p.name}</button>))}
        {analyst && setDrawing && <button data-testid="aoi-draw-toggle" onClick={() => setDrawing(!drawing)} className="inline-flex items-center gap-1 rounded border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider" style={{ borderColor: "rgba(157,78,221,0.6)", color: drawing ? "#F8FAFC" : "#9D4EDD", background: drawing ? "rgba(157,78,221,0.6)" : "transparent" }}><PenTool size={10} /> {drawing ? "drawing… (Esc)" : "draw custom AOI"}</button>}
      </div>
