    ...
    <div className="h-full overflow-y-auto p-6" data-testid="alerts-page">
      <div className="mb-6 flex items-end justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button data-testid="btn-refresh-alerts" onClick={load} className="inline-flex items-center gap-1.5 rounded border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider text-slate-300 hover:text-white" style={{ borderColor: "var(--border-highlight)" }}><RefreshCw size={12} /> Refresh</button>
        </div>
      </div>
      <div className="panel overflow-hidden" data-testid="alerts-list">
        <div className="grid gap-2 p-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          {shown.map((a) => (
            <div key={a.id} className="rounded border p-3 text-xs" style={{ borderColor: a.acknowledged ? "var(--border-default)" : "rgba(255,42,109,0.5)", background: a.acknowledged ? "transparent" : "rgba(255,42,109,0.05)" }} data-testid={`jobs-alert-${a.id}`}>
              <div className="flex items-center justify-between gap-2">
                <span className="flex items-center gap-1.5"><ShieldAlert size={12} color={a.severity === "high" ? "#FF2A6D" : a.severity === "medium" ? "#FFB703" : "#94A3B8"} /><span className="font-mono text-[10px] uppercase tracking-wider text-slate-400">{a.kind?.replace("_", " ")}</span></span>
