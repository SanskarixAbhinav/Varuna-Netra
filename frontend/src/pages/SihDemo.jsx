...
const Tag = ({ children, tone = "#38BDF8", testid }) => <span data-testid={testid} className="rounded px-1.5 py-0.5 font-mono text-[9px] font-bold uppercase tracking-wider" style={{ color: tone, border: `1px solid ${tone}66` }}>{children}</span>;
...
export default function SihDemo() {
  ...
  const steps = d ? [
    ...
    { t: "Sentinel-1 scene", icon: Satellite, body: <>
      <div className="mb-2 flex gap-2"><Tag tone="#10B981" testid="demo-scene-badge">{d.c.scene_status?.state === "SAR_READY" ? "REAL SENTINEL-1 · SAR READY" : d.c.scene_status?.state}</Tag><Tag>LATEST AVAILABLE ACQUISITION — not real-time</Tag></div>
    ...
    { t: "Spill candidate", icon: Waves, body: <>
      <div className="mb-2 flex gap-2"><Tag tone="#FFB703">EXPERIMENTAL SAR DARK-SPOT DETECTOR</Tag><Tag tone="#94A3B8">validation pending</Tag></div>
    ...
    { t: "AIS vessels", icon: Radio, body: <>
      <div className="mb-2 flex gap-2"><Tag tone={d.ais.state === "LIVE" ? "#10B981" : "#FFB703"} testid="demo-ais-state">AISStream now: {d.ais.feed || d.ais.state}</Tag></div>
      ...
      {d && (
        <div className="grid gap-4 lg:grid-cols-[220px_1fr]">
          <ol className="space-y-1" data-testid="demo-steps">
            {steps.map((s, i) => <li key={s.t}><button data-testid={`demo-step-${i}`} onClick={() => setStep(i)} className={`flex w-full items-center gap-2 rounded px-2 py-1.5 text-left font-mono text-[11px] ${i === step ? "bg-cyan-400/15 text-cyan-300" : "text-slate-400 hover:text-slate-100"}`}><s.icon size={12} /> {i + 1}. {s.t}</button></li>)}
          </ol>
