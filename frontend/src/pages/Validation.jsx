  ...
  <tr className="border-t" style={{ borderColor: "var(--border-default)" }}>
    ...
    <td className="px-4 py-2 font-mono text-xs">
      {value === "NOT YET VALIDATED"
        ? <span className="rounded px-2 py-0.5 text-[10px] uppercase tracking-wider" style={{ color: "#FFB703", background: "rgba(255,183,3,0.1)", border: "1px solid rgba(255,183,3,0.35)" }}>NOT YET VALIDATED</span>
    ...
    <div className="h-full overflow-y-auto p-6" data-testid="validation-page">
      ...
      {det && (
        <div className="panel mb-4 p-5 fade-up" data-testid="validation-detector">
          <div className="mb-2 flex items-center gap-2"><Cpu size={16} color="#00F0FF" /><h2 className="font-display text-lg font-semibold">Active detector</h2></div>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div><p className="label-mono">Mode</p><p className="mt-0.5 font-mono text-sm" style={{ color: det.ml_model_installed ? "#10B981" : "#FFB703" }}>{det.status_text}</p></div>
      ...
      {v && v.detection_metrics && (
        <div className="panel mb-4 overflow-hidden fade-up" data-testid="validation-detection">
          <div className="flex items-center gap-2 border-b px-4 py-3" style={{ borderColor: "var(--border-default)" }}><FlaskConical size={15} color="#00F0FF" /><h2 className="font-display font-semibold">Detection metrics</h2></div>
      ...
      {v && v.correlation_metrics && (
        <div className="panel overflow-hidden fade-up" data-testid="validation-correlation">
          <div className="flex items-center gap-2 border-b px-4 py-3" style={{ borderColor: "var(--border-default)" }}><BadgeCheck size={15} color="#00F0FF" /><h2 className="font-display font-semibold">Correlation metrics</h2></div>
          <table className="w-full text-sm">
            <thead><tr className="label-mono text-left"><th className="px-4 py-2 font-normal">Metric</th><th className="px-4 py-2 font-normal">Measured</th><th className="px-4 py-2 font-normal">Target</th></tr></thead>
            <tbody>
